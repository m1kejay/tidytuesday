...
```
    ...
    ## Warning in countrycode_convert(sourcevar = sourcevar, origin = origin, destination = dest, : Some values were not matched unambiguously: Individual Olympic Athletes, Kosovo
  ...
  text_transform(
   ...
   text_transform(
    ...
    fn = function(x){
      ...
      cleaned_df_gg$data %>% map(ggplot_image, height = px(30), aspect_ratio = 9.9)
    }
  ) %>%
  cols_hide(c(data)) %>%
  cols_label(
